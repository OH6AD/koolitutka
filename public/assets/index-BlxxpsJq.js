(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))s(n);new MutationObserver(n=>{for(const r of n)if(r.type==="childList")for(const g of r.addedNodes)g.tagName==="LINK"&&g.rel==="modulepreload"&&s(g)}).observe(document,{childList:!0,subtree:!0});function a(n){const r={};return n.integrity&&(r.integrity=n.integrity),n.referrerPolicy&&(r.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?r.credentials="include":n.crossOrigin==="anonymous"?r.credentials="omit":r.credentials="same-origin",r}function s(n){if(n.ep)return;n.ep=!0;const r=a(n);fetch(n.href,r)}})();class j{worker=new Worker(new URL("/assets/db.worker-C-JeQgBT.js",import.meta.url),{type:"module"});nextId=1;pending=new Map;constructor(){this.worker.onmessage=e=>{const a=this.pending.get(e.data.id);a&&(this.pending.delete(e.data.id),e.data.ok?a.resolve(e.data.data):a.reject(new Error(e.data.error)))}}init(e){return this.request({type:"init",dbUrl:e})}lookupCallsign(e){return this.request({type:"lookupCallsign",callsign:e})}searchSuggestions(e,a,s=8){return this.request({type:"searchSuggestions",query:e,mode:a,limit:s})}listChanges(e,a){return this.request({type:"listChanges",start:e,end:a})}request(e){const a=this.nextId++,s={...e,id:a};return new Promise((n,r)=>{this.pending.set(a,{resolve:n,reject:r}),this.worker.postMessage(s)})}}const B=1440*60*1e3;function S(){return I(new Date)}function E(t){const e=new Date;return e.setUTCDate(e.getUTCDate()-t),I(e)}function I(t){return t.toISOString().slice(0,10)}function F(t,e){if(t===null||e===null)return null;const a=Date.parse(`${t}T00:00:00Z`),s=Date.parse(`${e}T00:00:00Z`);return Number.isNaN(a)||Number.isNaN(s)?null:Math.max(0,Math.round((s-a)/B))}function N(t,e,a=!1){if(t===null)return"";const s=a?"> ":"";if(t<60)return s+e.days.replace("{n}",String(t));if(t<365)return s+e.months.replace("{n}",String(Math.max(1,Math.round(t/30))));const n=Math.floor(t/365),r=Math.round((t-n*365)/30);return s+e.yearsMonths.replace("{years}",String(n)).replace("{months}",String(r))}const Y={fi:{appTitle:"Koolitutka",appSubtitle:"Radioamatöörikutsujen tila ja historia",searchLabel:"Kutsu",searchPlaceholder:"Esim. OH2AD",searchButton:"Hae",prefixSearchOnly:"Hae vain kutsun alkuosaa",loading:"Ladataan tietokantaa...",error:"Virhe",available:"Vapaa",currentStatus:"Nykytila",history:"Historia",related:"Läheiset kutsut",changes:"Muutokset",from:"Alkaen",to:"Päättyen",update:"Päivitä",lastSevenDays:"Viimeiset 7 päivää",noRows:"Ei näytettäviä rivejä.",callsign:"Kutsu",status:"Tila",startDate:"Alkupäivä",since:"Alkaen",endDate:"Loppupäivä",active:"voimassa",date:"Päivä",change:"Muutos",duration:"Kesto",started:"alkoi",ended:"päättyi",metadata:"Päivitetty {updated}",language:"Kieli",close:"Sulje",noHistoryNotice:"Tällä kutsulla ei ole tunnettua historiaa. Se voi olla vapaa tai virheellinen. Tarkista kelpoisuussäännöt ennen sen hakemista.",validityRules:"Kelpoisuussäännöt",statusText:{VOIMASSA:"Voimassa",VARAUS:"Varaus",KARENSSI:"Karenssi",VAPAA:"Vapaa"},days:"{n} pv",months:"{n} kk",yearsMonths:"{years} v {months} kk"},sv:{appTitle:"Koolitutka",appSubtitle:"Status och historik för radioamatöranropssignaler",searchLabel:"Anropssignal",searchPlaceholder:"T.ex. OH2AD",searchButton:"Sök",prefixSearchOnly:"Sök endast från början",loading:"Laddar databasen...",error:"Fel",available:"Ledig",currentStatus:"Nuvarande status",history:"Historik",related:"Relaterade anropssignaler",changes:"Ändringar",from:"Från",to:"Till",update:"Uppdatera",lastSevenDays:"Senaste 7 dagarna",noRows:"Inga rader att visa.",callsign:"Anropssignal",status:"Status",startDate:"Startdatum",since:"Sedan",endDate:"Slutdatum",active:"aktiv",date:"Datum",change:"Ändring",duration:"Varaktighet",started:"började",ended:"slutade",metadata:"Uppdaterad {updated}",language:"Språk",close:"Stäng",noHistoryNotice:"Den här anropssignalen har ingen känd historik. Den kan vara ledig eller ogiltig. Kontrollera giltighetsreglerna innan du ansöker om den.",validityRules:"Giltighetsregler",statusText:{VOIMASSA:"I kraft",VARAUS:"Reserverad",KARENSSI:"Karenstid",VAPAA:"Ledig"},days:"{n} d",months:"{n} mån",yearsMonths:"{years} år {months} mån"},en:{appTitle:"Koolitutka",appSubtitle:"Amateur radio callsign status and history",searchLabel:"Callsign",searchPlaceholder:"E.g. OH2AD",searchButton:"Search",prefixSearchOnly:"Prefix search only",loading:"Loading database...",error:"Error",available:"Available",currentStatus:"Current status",history:"History",related:"Related callsigns",changes:"Changes",from:"From",to:"To",update:"Update",lastSevenDays:"Last 7 days",noRows:"No rows to show.",callsign:"Callsign",status:"Status",startDate:"Start date",since:"Since",endDate:"End date",active:"active",date:"Date",change:"Change",duration:"Duration",started:"started",ended:"ended",metadata:"Updated {updated}",language:"Language",close:"Close",noHistoryNotice:"This callsign has no known history. It may be available, or it may be invalid. Check the validity rules before applying for it.",validityRules:"Validity rules",statusText:{VOIMASSA:"Active",VARAUS:"Reserved",KARENSSI:"Cooldown",VAPAA:"Available"},days:"{n} d",months:"{n} mo",yearsMonths:"{years} y {months} mo"}},v=["fi","sv","en"];function W(t,e="fi"){for(const a of t){const s=a.toLowerCase().split("-")[0];if(v.includes(s))return s}return e}function d(t){return Y[t]}function Z(t,e){return t.replace(/\{([^}]+)\}/g,(a,s)=>String(e[s]??""))}function $(t){return t.trim().toUpperCase().replace(/\s+/g,"")}const C=/^\d{4}-\d{2}-\d{2}$/;function P(t){const e=t.startsWith("#")?t.slice(1):t,a=new URLSearchParams(e),s=a.get("lang"),n=a.get("start"),r=a.get("end"),g=$(a.get("q")??"");return{q:g.length>0?g:null,start:n&&C.test(n)?n:null,end:r&&C.test(r)?r:null,language:s&&v.includes(s)?s:null}}function H(t){const e=new URLSearchParams,a=$(t.q??"");return a.length>0&&e.set("q",a),e.set("start",t.start),e.set("end",t.end),e.set("lang",t.language),`#${e.toString()}`}function z(t){return t!=="false"}const G="https://oh2ti.fi/wp-content/uploads/2023/05/PRK-RA2023_L1-L2_K-moduuli.pdf#page=9",O="prefixSearchOnly",b=new j,k=P(location.hash),L=localStorage.getItem("language");let o=k.language??(L&&v.includes(L)?L:null)??W(navigator.languages),u=k.start??E(7),c=k.end??S(),l=k.q,m=z(localStorage.getItem(O)),A=null,h=null,w=[],D=!0,y=null;const K=document.querySelector("#app");if(!K)throw new Error("Missing app root");window.addEventListener("hashchange",()=>{nt(location.hash)});i();b.init("/koolitutka.sqlite").then(t=>(A=t,D=!1,p(),U())).catch(t=>{D=!1,y=t instanceof Error?t.message:String(t),i()});function i(){const t=d(o);document.documentElement.lang=o,K.innerHTML=`
    <main class="shell">
      ...
      ${y?`<p class="notice error">${t.error}: ${st(y)}</p>`:""}
  ...
  `}function X(t){if(t.history.length>0)return"";const e=d(o);return`<p class="status-note">${e.noHistoryNotice} <a href="${G}" target="_blank" rel="noopener noreferrer">${e.validityRules}</a>.</p>`}function x(t){const e=d(o);return t.length===0?`<p class="empty">${e.noRows}</p>`:`
  ...
  `}function tt(t){const e=d(o);return t.length===0?`<p class="empty">${e.noRows}</p>`:`
    <div class="table-wrap">
      <table>
        ...
        <tbody>${t.map(a=>`
          <tr>
            ...
            <td><a class="callsign-link" href="${at(a.callsign)}">${a.callsign}</a></td>
            <td>${T(a.status)}</td>
            <td>${R(a)}</td>
            <td>${_(a)}</td>
  ...
  `}function q(){return b.listChanges(u,c).then(t=>{w=t,i()})}function et(t){const e=d(o),a=[];t.from_date&&a.push(`<p>${e.since}: ${R(t)}</p>`),t.to_date&&a.push(`<p>${e.endDate}: ${_({to_date:t.to_date})}</p>`);const s=t.to_date??S(),n=N(F(t.from_date,s),e,t.from_date_estimated);return n&&a.push(`<p>${e.duration}: ${n}</p>`),a.join("")}function V(t){return l=t,b.lookupCallsign(t).then(e=>{h=e})}function U(){return q().then(()=>{if(l===null){h=null,i();return}return V(l).then(i)})}function at(t){return H({q:t,start:u,end:c,language:o})}function p(){const t=H({q:h?.callsign??l,start:u,end:c,language:o});location.hash!==t&&history.replaceState(null,"",t)}function nt(t){const e=P(t),a=l;o=e.language??o,u=e.start??E(7),c=e.end??S(),l=e.q,U().then(()=>{l!==null&&l!==a&&window.scrollTo({top:0,behavior:"smooth"})}).catch(f)}function R(t){return t.from_date===null?"":t.from_date_estimated?`< ${t.from_date}`:t.from_date}function _(t){return t.to_date==="NOW"?d(o).active:t.to_date}function T(t){return d(o).statusText[t]}function f(t){y=t instanceof Error?t.message:String(t),i()}function st(t){return t.replace(/[&<>"]/g,e=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;"})[e]??e)}
